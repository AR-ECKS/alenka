...
@section('content')
    ...
    <section>
        <div class="card">
            ...
            <form method="POST" action="{{ route('despacho.update', $despacho->id) }}" enctype="multipart/form-data"
                class="my-4 mx-4">
                {{ method_field('PATCH') }}
                {{ csrf_field() }}
                <div class="row">
                        ...
                        <div class="input-group mb-2">
                            <div class="input-group-prepend">
                                <div class="input-group-text">
                                    <i class="fas fa-user-edit" style="padding-right:5px"></i><b>{{ 'Entregar a' }}</b>
                                </div>
                            </div>
                            <select class="form-control" id="usuario" name="receptor" required>
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ isset($despacho->user_id) && $despacho->user_id == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }} - {{ $user->primary_role ? $user->primary_role->name : 'Sin rol' }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-sm-4">
                        <div class="input-group mb-2">
                            ...
                            <input type="text" class="form-control" maxlength="50" id="codigo" name="codigo" readonly value="{{ isset($codigo) ? $codigo : '' }}">
                        </div>
                    </div>
                    ...
                    <div class="col-sm-4">
                        <div class="input-group mb-2">
                            ...
                            <input type="text" class="form-control" id="fecha" name="fecha" value="{{ \Carbon\Carbon::now()->format('d-M-Y') }}" readonly>
                        </div>
                    </div>


                    <div class="col-sm-6">
                        <table id="tablaProductos" style="font-size: 14px" class="table table-sm table-responsive table-striped table-bordered pt">
                            ...
                            <tbody>
                                ...
                                <tr id="producto_{{ $index }}">
                                    ...
                                    <td id="stock_presentacion_{{ $index }}"><b>{{ $materia_prima->stock_presentacion }}</b> {{ $materia_prima->presentacion }}s</td>
                                    <td id="stock_unidad_{{ $index }}"><b>{{ $materia_prima->stock_unidad }}</b> {{ $materia_prima->unidad_medida }}</td>
                                    <td>
                                        <input type="text" class="form-control" id="presentacion_{{ $index }}" data-cantidad="{{ $materia_prima->cantidad }}" oninput="calcularUnidad({{ $index }})">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control" id="unidad_{{ $index }}" data-cantidad="{{ $materia_prima->cantidad }}" oninput="calcularPresentacion({{ $index }})">
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-sm btn-success agregar-btn" onclick="agregarProducto({{ $index }}, '{{ $materia_prima->id }}', '{{ $materia_prima->nombre }}', '{{ $materia_prima->presentacion }}', '{{ $materia_prima->unidad_medida }}', {{ $materia_prima->cantidad }}, {{ $materia_prima->stock_presentacion }}, {{ $materia_prima->stock_unidad }})" style="display: {{ $materia_prima->stock_presentacion == 0 || $materia_prima->stock_unidad == 0 ? 'none' : 'inline-block' }};">
                    ...
                    <div class="col-sm-6">
                        <table id="tablaCarrito" style="font-size: 14px" class="table table-sm table-responsive table-striped table-bordered pt">
                            <thead class="text-sm thead-dark" class="inventario">
                                <tr>
                                    <th>Producto</th>
                                    <th>Presentacion</th>
                ...
                <div class="col-sm-12">
                    <div class="input-group mb-2">
                        ...
                        <input type="text" class="form-control" maxlength="50" id="observacion" name="observacion" value="{{ isset($despacho->observacion) ? $despacho->observacion : '' }}">
                    </div>
                </div>
...
Actualizar                    </button>
                ...
                <script>
                    ...
                    function agregarProducto(index, id, nombre, presentacion, unidad_medida, cantidad, stock_presentacion, stock_unidad) {
                    let presentacionInput = document.getElementById(`presentacion_${index}`);
                    let unidadInput = document.getElementById(`unidad_${index}`);
                    let presentacionValue = parseFloat(presentacionInput.value);
                    let unidadValue = parseFloat(unidadInput.value);
