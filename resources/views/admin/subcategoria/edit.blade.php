...
@section('content')
    ...
    <section>
        <div class="card">
            ...
            <form method="POST" action="{{ route('subcategorias.update', $subcategorium->id) }}"  enctype="multipart/form-data" class="my-4 mx-4">
                {{ method_field('PATCH') }}
                                {{ csrf_field() }}
                                @include ('admin.subcategoria.form', ['formMode' => 'edit'])
                                </form>
        </div>
    </section>
@endsection
